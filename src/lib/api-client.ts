...
export const categoriesApi = {
  ...
  getAllWithSubcategories: () => apiClient.get('/categories/with-subcategories'),
  getAllSubcategories: () => apiClient.get('/categories/subcategories'),
  getHierarchy: () => apiClient.get('/categories/hierarchy'),
  getById: (id: string) => apiClient.get(`/categories/${id}`),
  getWithChildren: (id: string) => apiClient.get(`/categories/${id}/with-children`),
  getSubcategories: (id: string) => apiClient.get(`/categories/${id}/subcategories`),

  // Admin endpoints
  create: (data: FormData) => apiClient.post('/admin/categories', data, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  }),
  update: (id: string, data: FormData) => apiClient.put(`/admin/categories/${id}`, data, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  }),
  ...
  updateSubcategory: (categoryId: string, subId: string, data: FormData) => 
    apiClient.put(`/admin/categories/${categoryId}/subcategories/${subId}`, data, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    }),
