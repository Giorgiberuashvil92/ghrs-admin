...
const uploadToCloudinary = async (file: File | Blob, folder: string = 'articles'): Promise<string> => {
  try {
...
export async function getArticles(filters?: ArticleFilters, page: number = 1, limit: number = 20) {
  try {
    ...
    const transformArticle = (article: any) => {
      ...
      return {
        ...
        category: typeof article.categoryId === 'object' ? article.categoryId : undefined,
        blogId: typeof article.blogId === 'string' ? article.blogId : article.blogId?._id || '',
...
export const getArticleById = async (id: string): Promise<Article> => {
  try {
    ...
    return {
      ...
      categoryId: categoryId,
      category: typeof data.categoryId === 'object' ? data.categoryId : undefined,
      blogId: typeof data.blogId === 'string' ? data.blogId : data.blogId?._id || '',
  ...
  try {
    ...
    if (request.isFormData) {
      ...
      if (data.tableOfContents && data.tableOfContents.length > 0) {
        formDataToSend.append('tableOfContents', JSON.stringify(data.tableOfContents));
      }
      ...
      if (data.featuredImages && data.featuredImages.length > 0) {
        formDataToSend.append('featuredImages', JSON.stringify(data.featuredImages));
      }
...
async function sendFormDataRequest(id: string, formData: FormData): Promise<Article> {
  const response = await fetch(`${API_BASE_URL}/articles/${id}`, {
    method: 'PATCH',
    credentials: 'include',
    body: formData
