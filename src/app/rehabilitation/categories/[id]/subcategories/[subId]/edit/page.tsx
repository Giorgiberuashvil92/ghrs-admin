...
export default function EditSubCategoryPage({
  ...
  const [formData, setFormData] = useState({
    name: { en: "", ru: "" } as LocalizedString,
    description: { en: "", ru: "" } as LocalizedString,
    imageUrl: "",
    isActive: true,
    isPublished: false,
    sortOrder: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile || (formData.imageUrl && formData.imageUrl.trim())) {
        ...
        dataToSend.append("name", JSON.stringify({
          ka: formData.name.en, // Use English name as Georgian fallback
          en: formData.name.en,
          ru: formData.name.ru
        }));
        dataToSend.append("description", JSON.stringify({
          ka: formData.description.en || "", // Use English description as Georgian fallback
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                <label className="mb-2 block text-sm font-semibold text-gray-700">
                  {t("nameInEnglish")} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="mb-2 block text-sm font-semibold text-gray-700">
                  {t("nameInRussian")} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  required
