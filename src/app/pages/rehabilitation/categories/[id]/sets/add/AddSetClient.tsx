...
import { createSet, type CreateSetData } from '@/lib/api/sets';
...
export default function AddSetClient({ category }: AddSetClientProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError('სეტის შექმნა ვერ მოხერხდა. გთხოვთ სცადოთ თავიდან.');
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-gray-900">სეტის დამატება - {category.name.ka}</h1>
        <p className="mt-2 text-sm text-gray-700">
          შეავსეთ ფორმა სეტის დასამატებლად
        </p>
      </div>
