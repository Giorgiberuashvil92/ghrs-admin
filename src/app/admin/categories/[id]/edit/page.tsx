...
async function updateCourseCategoryWithFile(
  id: string,
  formData: FormData
): Promise<CourseCategoryType> {
  const res = await fetch(`${API_BASE_URL}/api/course-categories/${id}`, {
    method: "PATCH",
    credentials: "include",
    body: formData,
  });
...
export default function AdminEditCategoryPage({ params }: PageProps) {
  ...
  useEffect(() => {
    getCourseCategoryById(resolvedParams.id)
      .then((data) => {
        setCategory(data);
        setFormData({
          name: { en: data.name?.en ?? "", ru: data.name?.ru ?? "" },
          description: { en: data.description?.en ?? "", ru: data.description?.ru ?? "" },
          imageUrl: data.image || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (imageFile || (formData.imageUrl && formData.imageUrl.trim())) {
        ...
        else if (formData.imageUrl?.trim() && /^https?:\/\//.test(formData.imageUrl))
          dataToSend.append("imageUrl", formData.imageUrl);
        await updateCourseCategoryWithFile(category._id, dataToSend);
      } else {
        await updateCourseCategory(category._id, {
          name: formData.name,
